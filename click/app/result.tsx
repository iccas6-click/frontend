...
export default function ResultScreen() {
  ...
  const { photoUri, category, prevItems } = useLocalSearchParams<{
    photoUri?: string;
    category?: string;
    prevItems?: string;
  }>();

  const parsedPrevItems: RecognizedItem[] = prevItems
    ? JSON.parse(prevItems)
    : [];
  ...
  useEffect(() => {
    if (recordId && !loading && !error) {
      updateScan(recordId, items).catch((e) =>
        console.warn('기록 갱신 실패:', e),
      );
    }
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.listContent}>
        {loading ? (
          ...
          items.map((item, i) => <Text key={i}>{item.name}</Text>)
        )}
      </ScrollView>
      <View style={styles.footer}>
        {selectedCategory === '알약' ? (
          ...
          <Pressable
            style={styles.cta}
            onPress={() => {
              const finalItems = [...parsedPrevItems, ...items];
              router.push({
                pathname: '/analyze',
                params: { items: JSON.stringify(finalItems) },
              });
            }}
          >
